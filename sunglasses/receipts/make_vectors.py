...
# T9 ruling 60: the hook log's genesis marker, keys/log-hook.genesis. Its
# chain_id is 32 lowercase hex and its key_id (added in build) the seed's real
# fingerprint, as the writer writes them, so the verifier's shape checks hold.
MARKER = {
    "chain_id": "00112233445566778899aabbccddeeff",
    "event": "log_genesis",
    "log": "hook",
    "t_wall_ns": 1758700000000000000,
    "wire": wire.WIRE_VERSION,
}
...
def build() -> dict:
    ...
    marker = dict(MARKER, key_id=wire.key_fingerprint(public))
    marker_bytes = wire.marker_signing_bytes(marker)
    marker_signature = private.sign(marker_bytes)
    marker_signed = dict(marker, signature=marker_signature.hex())
    marker_line = wire.encode(marker_signed)
    ...
    return {
        ...
        "domains": {
            ...
            "marker": wire.MARKER_DOMAIN.decode("latin-1"),
            "marker_hex": wire.MARKER_DOMAIN.hex(),
        },
        "key": {
        ...
        "records": {
            ...
            "marker": {
                "record_unsigned": marker,
                "signing_bytes_hex": marker_bytes.hex(),
                "signature_hex": marker_signature.hex(),
                "record_signed": marker_signed,
                "line_hex": marker_line.hex(),
                "line_utf8": marker_line.decode("utf-8"),
            },
        },
