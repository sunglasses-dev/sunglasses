...
### Fixed
- **A 27 KB document that is one long word took 255 seconds to scan, and now takes 0.03.** Not a 1 MiB adversarial payload: 27 kilobytes, the size of a minified JS file or a base64 blob in a tool result, reachable by accident and on purpose in shipped 0.5.7. One rule owned all of it, `GLS-ENC-ALT-210`, through an unbounded greedy run `[A-Za-z0-9+/]{40,}` that walked 26,977 characters and backtracked from every start position toward a literal the document does not contain. The reason the prefilter could not skip the rule outright is the part worth keeping: the rule was ONE regex with three alternatives, and the middle one is a bare braille character class carrying no literal, so a single underivable branch dropped the required-literal clause for the whole regex and kept the base64 branch unskippable. The three alternatives are now three regex entries and the greedy run is bounded. Nothing was widened, nothing removed, no engine code changed, and detection is identical across 4,620 compared cells (76 real READMEs on seven channels, both reviewer input sets, and a positive for each alternative). `tests/test_long_word_scan_cost.py` carries the document as a 2 s budget and asserts the branches stay separated, because re-merging them silently restores the 255 seconds.
