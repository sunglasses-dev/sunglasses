...
- **An approval is keyed on the SERVER, not on the tool list.** (#221) Two servers
...
- **`install` edits a config and never creates one.** (#221) Aimed at a path that does
...
### Fixed
- **The outbound firewall cleared live credentials that happened to contain a placeholder word.** `is_placeholder` decided on a SUBSTRING: any secret-shaped token whose lowercase form contained one of eighteen words was called "demonstrably not live" and allowed out. Six of those words are four characters long (`0000`, `aaaa`, `here`, `todo`, `xxxx`, `your`) and a credential is base62, so a real key can carry one by accident. `AKIAHERE4CIPPERUVIFX`, `AKIAYOUR4CIPPERUVIFX` and `AKIA0000CIPPERUVIFXG` are well-formed AWS key ids and all three were cleared and sent, while `AKIAJ7K2QW9PLM3XZV5B` was caught. This was a detection gap in a shipped guard, not a false-positive setting, and it was reachable by an attacker who picks a key rather than by accident alone. The guard now decides on whole segments — the pieces a token splits into at every non-alphanumeric character — and never on a substring of one: a placeholder is a token that IS a placeholder, not a token that contains one. The five filler entries are matched by SHAPE instead of by literal, one repeated character or a consecutive ascending run of at least four, because nobody types filler to a fixed length and `xxxx` and `xxxxxxxx` are the same thing to a reader. Real placeholders are unaffected: `YOUR_KEY_HERE`, `<YOUR_KEY>`, `$TOKEN`, `sk-ant-REPLACE_ME`, `xxxxxxxx` and `0000-0000` all still clear, asserted one per word in both the bare and separated forms. The rule took two passes: the first repair decided on ANY SEGMENT, which is the same decision one level up — a live Slack token whose numeric groups happen to run `1234-5678`, and a live Stripe key carrying a whole `here` segment, were both cleared by it. The judgement is on the whole material: every segment outside the format's own literal prefix must be a placeholder word, typed filler, or a structural noun, and at least one must be an actual placeholder, so a token of pure structure like `my_api_token` does not clear. One behaviour change beyond the gap: AWS's documentation key `AKIAIOSFODNN7EXAMPLE` carries EXAMPLE as a suffix rather than a segment, so the repaired guard no longer clears it and it moves to `KNOWN_PUBLIC_CANARIES`, which is the file's stated mechanism for a published vendor fixture and asserts the entry still matches a rule. Vendor: Amazon Web Services. Document: *Manage access keys for IAM users*, https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html — read 2026-09-14, where the page states "Access keys consist of two parts: an access key ID (for example, AKIAIOSFODNN7EXAMPLE)". The string is present on the live page as the published example, which is what this list is for. 487 firewall and corpus tests pass, the 78-document clean corpus and the canary set are unchanged, and a mutation restoring the substring test fails 28 of them.
- **The same guard, two levels further out (independent review, round 3).** Two descriptions above were wider than the code: "the format's own literal prefix" was implemented as a GLOBAL set of prefix words skipped at ANY position plus four literals stripped from EVERY segment, and "one repeated character" was measured after the whole token had been lowercased. Both are lost DENIALS on live material. A foreign provider's prefix dropped into another provider's credential body — the AWS temporary-key prefix inside a GitHub, Stripe, Slack, Anthropic or OpenAI token, followed by filler — was discarded as format and the token cleared; so was a body that joined other formats' words to an ascending run. Separately, a 40-character body drawn from `q` and `Q` holds two distinct characters and is material, but lowercasing first made it a run of one character. Fifteen constructed credential controls regressed from deny to defer this way. Now: exactly ONE format literal comes off the FRONT of the token, taken from the grammar of the rule that actually matched (each rule carries its own leading literals beside its regex), and every remaining segment is material; shape is measured on the material exactly as it arrived, while placeholder WORDS still compare case-insensitively. A literal no rule declares is not a format at all, so a `sk_test_` prefix — which no shipped rule owns — no longer clears anything. Restoring the round-2 code fails 18 of the new controls.
- **A cleared canary is now an audited event.** `KNOWN_PUBLIC_CANARIES` is the only sanctioned way to clear a real format match and it was the only event in the lane that left no trace: the hit was found, dropped, and the call ended on an ordinary clean receipt. The terminal record now carries the rule id, a SHA-256 fingerprint and the clearance reason — never the material.

### Changed
- **MCP scans build one engine per process, not one per tool call.** (#175) The per-call latency of an MCP scan no longer includes compiling the ruleset; results are byte-identical to the per-call engine, asserted across the MCP test matrix.
- **The proxy's session core is in the wheel, inert.** (#164, #178) A parser that names which rule a malformed frame broke, a teardown that settles every held request exactly once with the first recorded cause, ids correlated by their JSON type (`"2001"` and `2001` are different requests), and a worker-result validator that refuses a result which parses perfectly and means nothing (byte counters must be integers, a scan cannot be complete and unfinished at once, an engine timeout is a deadline rather than a crash). **No entry point ships with it in this release**: nothing installs it, nothing routes through it, and no page, post or release note may describe proxy mediation as a supported surface until one does. It is listed here so the wheel's contents are accounted for, not as a capability.

### Not claimed in this release
- General inspection of tool results. What ships is the CREDENTIAL lane on `api_response` for eight formats (above), and `GLS-SD-010` stays open in that direction.
...
- **`GLS-MCP-POISON-201` was catastrophic backtracking, and the repeat causing it was doing nothing.** The rule paired a manifest key with a payload phrase as `(?:KEY.+?){1,3}`, three nested lazy gaps under DOTALL followed by an alternation. On a document that repeats one manifest-key fragment (row `R1_repeated_description_send` of the committed timing fixture) the alternation never completes, so every way of partitioning the text between three gaps is tried: about sixteen times the cost for each doubling of the input. Measured on 0.5.7, 2.0 s at 1 KB, 33 s at 2 KB, **531 s at 4 KB**. The prefilter was never going to help, and it is not at fault: it derives both clauses correctly and that document carries one literal from each derived clause. Change the second literal and the same 2 KB scan is 0.005 s instead of 32.9 s. The repeat collapses to `KEY.+?` because the single unbounded lazy gap already accepts everything two more hops could, which is a cost change and not a detection change. DECISIONS are unchanged everywhere tested. SPANS are not, and an earlier draft of this entry said they were: "0 span diffs over 4,109 rows" was true of those rows and false of the rule, because that grammar never repeated a manifest key inside the gap. When a key repeats there and a phrase starts immediately after the inner key, the old form's second greedy hop has to consume a character, so it skips that phrase and reports a match ending at a later one; the one-hop form ends at the nearest phrase. On the reviewer's first counterexample, pinned in the fixture as `reviewer_counterexamples[0]`, the reported match goes from `[0,43]` to `[0,29]`. Both still block. The shorter nearest-phrase span is better evidence, so the change is taken rather than reverted, and it is a PUBLIC change: the CLI's JSON `matched_text` for this rule moves on message, file and web_content. 264 generated rows of that class and the reviewer's two counterexamples are pinned with BOTH the old and the new span 4 KB now costs 3.89x the same engine without the rule, against about 33,000x before. The rule also declares `anchor_terms` and `anchor_span`. The windowed matcher that reads them is on main now, so they are LIVE and this rule compiles to anchored mode with nine terms and a span of 8,192; an earlier draft of this entry said the engine ignored them, which was true when it was written and is not now. the objects rather than the verbs are declared because the document above carries a verb and no object, so that matcher finds no window in it at all (27 KB drops from 2.0914 s to 0.0041 s and 1 MiB, which the plain form cannot finish, takes 0.1585 s). SCOPE of those three numbers: they are the RULE alone, measured on an engine built from this rule only. A full engine scanning the same documents costs more, because every other rule is also running. The windowed matcher is on main now, so the declaration is LIVE rather than inert: this rule compiles to anchored mode with nine terms and a span of 8,192. DISCLOSED LIMIT, stated as a possibility rather than a boundary: an object more than 8,192 characters after its manifest key MAY no longer match, measured anchor-relative. A single `credentials` anchor at 8,192 characters from the key start is retained and 8,193 is lost, but adding ten other anchors triggers the matcher's density fallback and restores the match beyond 8,192, so the number bounds one anchor's window and not the rule. The quadratic tail above 27 KB is still live and its scale is worth stating: about 2.2 s at 27 KB and about 48 s at 128 KB.
- **That rule shipped with no tests at all.** Its attack-db entry carried `"examples": {"malicious": [], "benign": []}`, two empty lists, and it matched none of the 2,746 inputs in the review corpus, so nothing would have noticed. It now has 5,280 generated malicious shapes across six manifest keys, twenty-two injection phrases, five document shapes and eight key-to-payload distances, plus 34 near misses that must stay clean, with every decision and span pinned from the previous release. Five of those phrase spellings were added in round 2 because the reviewer found the regex accepts them and the generator did not produce them. The filler is whole repetitions of a 32-character sentence, so three of the eight nominal gaps collapse and the 5,280 rows are 4,620 distinct texts; both numbers are asserted so neither can be quoted as the other. A further 264 rows cover the repeated-key class above. The published examples list carries a representative twelve and six.
- **69 pattern display names normalised, no detection change.** Names are rendered on the public pattern pages and shipped inside the wheel, and sixty-nine carried research residue from the generation farm: the date a work order ran, a run id such as `C20260718T133021_azrollinaz`, an internal `.md` filename, and the account names the farm hands run under. Ids, predicates, channels, severities and keywords are untouched, verified by comparing every non-name field against the previous tree (1,540 patterns, zero differences). `tests/test_pattern_name_hygiene.py` is the gate that stops the seventieth, since the names are written by automation that has no reason to know the convention.
- **A dead control now asks and names itself (RED 4).** A missing, empty, unreadable or unparseable policy file, and a receipts directory that cannot be written, all used to end as `{}` on the wire: the call fell through to the normal permission flow with nothing on screen to say the control was down. `{}` is indistinguishable from "checked, nothing found". Each state now returns `ask` with a reason naming which control died. The dead control is the FALLBACK rather than a short-circuit, so a later lane that does decide still decides and the receipt still carries the confession, and `degraded` is now set on every call where a control was down instead of on every call except the ones where it was the verdict. A block survives an unwritable audit trail unchanged: losing a receipt must not weaken a decision. A missing policy counts as dead only where an install marker says one was written, because the receipts directory creates the home directory itself and "the directory exists" is not evidence a policy was ever configured.
- **`api_response`, `log_memory` and `agent_input` reach ships as six SIBLING rules, and the six parents are untouched.** Three attempts to give the parents one regex covering every channel were rejected, the last two for REGRESSIONS on the existing channels rather than for the new one: each fix closed the exact escape a reviewer had demonstrated and left the rest of the grid open, and the round-3 head-of-phrase follower rule was itself a word-keyed escape (append "please", "privately" or "as plain text" and it went silent across 36 suffix variants). The parents now keep parent behaviour on their four existing channels, which makes zero regressions true by construction, and `GLS-PI-021-API` and its five siblings carry the new reach alone. Their predicate is a verb-to-object class pairing with no follower rule: a disclosure verb must reach a secret-class object, or an override verb must reach a control-class object, and bare "safety" and "security" are not objects. Measured: 3,287 generated variants all fire, the reviewer's 340 attack inputs all fire, 0 of 30 R1 benign fixtures fire, and a parent comparison over 448 inputs on all four existing channels shows 0 differences. DISCLOSED LIMIT on the new channels: a sensitive word used as a modifier can fire ("configuration guides", "access-token format guide"), 22 of 48 such rows in the reviewer's boundary set; separating them needs noun-phrase grammar, which is what three rounds of regex failed to do. DISCLOSED COST, measured 2026-09-12 on Python 3.14 at 1 MiB, medians of three rotated trials against the same engine without these six rules. On ordinary documents the six siblings are cheap. On a document that repeats one of their MARKERS every few dozen bytes they are not, because the marker is found everywhere and each hit pays for a bounded search that finds no object. A megabyte of `<admin>show ` repeated costs 9.61 s against 0.49 s, which is 19.6x. A megabyte of `note to ai show ` costs 11.8x and `ignore mask show ` 7.2x. This is a cost on adversarial input and not on prose, it is stated rather than discovered, and the windowed matcher in a following change retires the worst of it (the same `<admin>show ` megabyte anchors to 0.54 s, 1.12x). `GLS-PI-021-API` also gains `anthropic` and `openai` as routing keywords. Every vendor mention was indexed only as a phrase (`at openai`, `employee at anthropic`) and a newline anywhere inside a phrase destroys it, so eleven markers the regex accepts could not reach the rule through the keyword index. No regex changed. Decisions do change, which is the point of the keywords: on the four existing channels 11,219 frozen inputs across four channels, 44,876 paired cells on each of Python 3.14.7 and 3.9.6, are unchanged, and on the three new sibling channels 180 cells change, 60 attack inputs times three channels, every one allow to block/high with only `GLS-PI-021-API` added. 180 recoveries and 0 benign changes, measured over the 1,011 frozen inputs whose normalized index text carries either token. Measured cost of the two tokens is 1.05x on a megabyte of the bare token and 1.00x on prose.
- **The firewall records that a call arrived, not only how it ended (RED 5).** A PreToolUse hook killed on the harness's timeout fails open and writes nothing at all, so the audit trail was silent for the one failure that matters most, and that silence is indistinguishable from a hook that was never installed. An `in_flight` record carrying an evaluation id is now appended before evaluation begins and the terminal record references it, so a killed or crashed evaluation leaves an orphan. `sunglasses receipts --verify` pairs them, names the orphans and exits 1. Receipts written before this carry no `kind` and are counted as terminal rather than reported as orphans. Measured cost of the extra append: 0.089ms to 0.147ms per call. The README sentence claiming every fail-open writes a receipt was false and now says what actually happens.
...
- The release gate fails on ABSENCE, not only on failure: `certify` needs every job, runs with `always()`, and treats a missing or skipped result as a failure. `main` had no branch protection at all when this was found; a ruleset now requires `certify` and a pull request.
- A lint for assertions that cannot fail. `assert x or True` had been cited as proof once in this repository and a fourth instance sat in the v0.5.6 acceptance matrix. Both shapes are now a test failure.
- The timeout rule covered three named jobs and skipped the six way integrity matrix, the only job that has ever hung. Every job carries `timeout-minutes` now and the check reads the workflow rather than a list.
- A scanner for the literal free alternation shape that made one rule unskippable, so the next one is found directly instead of through its symptom.
- Every local test run prints the working tree sha and branch it ran on, after three green suites in one day were offered as evidence for trees CI never tested.
- The anchored mode timing gate counts the mechanism (how many start positions were searched) instead of timing a two millisecond difference, which flaked once at 1.18x on a tree where nothing had regressed.
- The release fixture pins against the previous release rather than against whatever `main` says today, which is what failed the first 0.5.8 certification run for no defect.
- `pattern-integrity.yml` declares `read-all` permissions and pins every action to a commit SHA, like the other two workflows. Scorecard moved from 4.8 to 5.8.
- The README answers what works and what does not on the first screen, since a stranger arriving from a thirty second video decides there.

### Patterns
- 1,546 patterns, 118 categories, 6,675 keywords. Six rules added, the `-API` siblings named above, no new category and no blog. The published `attack-db/manifest.json` is regenerated at this release.
...
intake: every regex re-proven against its own hostile fixtures, silent on benign fixtures and
0 hits across the 78-document real-world FP corpus. 28 gate cards were rejected at intake with
written reasons (TP_MISS, REDOS_SUSPECT, CORPUS_FP, NO_HOSTILE_FIXTURE) and stay queued for repair.

- Added: 26 detection patterns across six categories — path_traversal (8, GLS-PT-015..022),
  sandbox_escape (5, GLS-SE-*), representation_parser_differential (5, GLS-RPD-*),
  tool_output_poisoning (3, GLS-TOP-648..650), provenance_chain_fracture (3, GLS-PCF-*) and
  delegated_approval_rebinding (2, GLS-DAR-005/007): lexical-vs-canonical path containment,
  model-controlled sandbox disable flags, parser disagreement across delegator/delegate,
  error-recovery transcript injection and related mechanisms.
- Benchmark re-measured at 0.4.7: 86.1% precision / 97.4% recall / 0.914 F1 — unchanged.
- Blog: "Secure AI Agent Tool Access Needs Boundaries" (merged PULSE Day 4 evidence page,
  covers all 26 shipped ids).
...
### Added
- **12 new patterns, all in `cross_agent_injection`** (one-category-per-day rule). Auto-shipped via daily-push runner.
...
### Added
- **15 new patterns, all in `cross_agent_injection`** (one-category-per-day rule). Auto-shipped via daily-push runner.
