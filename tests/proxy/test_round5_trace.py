...
def test_RC14b_trace_after_record_retirement(tmp_path,monkeypatch,close_first):
 boundary="record_retired"
 ...
 with peer(tmp_path,responses,requests,linger=True) as child:
  ...
  target=start+next(i for i,line in enumerate(lines) if line.strip()=='return self._closed is None')
