...
MUTATIONS = [
    ...
    ('C6-CMD', 'WRAPPED without checking the command that would run',
    ...
    ('R5-RECFIELD', 'a record missing a contract field is accepted',
     '    missing = [f for f in RECORD_FIELDS if f not in record]',
     '    missing = []',
     'test_uninstall_refuses_a_record_it_cannot_read'),
    ('COLLISION', 'a record is keyed by name alone, not bound to its target',
