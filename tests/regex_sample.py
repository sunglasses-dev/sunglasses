...
NOTE: this is the same renderer as `tests/regex_sample.py` on the
`feat/sd010-embedded-boundary` branch, where it generates probes for the
attack-db semantic-equality test. Two copies exist because the branches are
independent; when both land, ONE of them should move to a shared home and the
other should import it. Written down here so the duplication is a decision
someone can act on rather than a thing they discover.
