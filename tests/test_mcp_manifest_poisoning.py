...
def _main_source():
    """Main's regex for this rule, from the fixture's stored bytes.

    Round 2 rebuilt it by string-editing this head's source and broke the moment
    round 3 changed the gap as well. Round 3 read it out of git, which was right
    on a developer checkout and failed five tests on CI, whose fast and matrix
    checkouts are depth 1 with no origin/main ref. So the fixture carries the
    bytes verbatim next to the hash it always carried; this returns them after
    checking the hash, and `test_the_stored_main_regex_is_mains_actual_regex`
    compares them to git whenever git can answer. Deterministic on every
    checkout, and still not a reconstruction.
    """
    stored = PINNED["main_regex"]
    digest = hashlib.sha256(stored.encode("utf-8")).hexdigest()
    assert digest == PINNED["main_regex_sha256"], (
        "the fixture's stored main regex does not hash to its recorded sha256; "
        "the fixture was edited by hand")
    return stored


def test_the_stored_main_regex_is_mains_actual_regex():
    """The fixture is not a mirror: its stored main regex must be the bytes on
    the merge-base with origin/main. Skips, by name, where git cannot answer,
    which is exactly the checkout this test cannot help on."""
    ...
    assert live == _main_source(), (
        "main's regex for this rule changed since the fixture was pinned; "
        "re-pin the rows against the new main, do not edit the stored regex")


def _three_hop():
    """Main's form. Not rebuilt, READ."""
    source = _main_source()
    assert source != SOURCE, "main and this head carry the same regex"
