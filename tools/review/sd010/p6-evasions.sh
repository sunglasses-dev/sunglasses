#!/bin/zsh
# P6 the six inputs round 1 forced us to find. Each must BLOCK on head and go
# QUIET again under the pre-built tree that restores its specific defect --
# M8 for the terminator gap, M9 for the prefix gap. Closed is not enough; the
# row must be shown to be doing the closing.
...
D=$(cd $(dirname $0)/.. && pwd)
PYTHONUNBUFFERED=1 python3 "$D/probes/_p6.py" "$D"
